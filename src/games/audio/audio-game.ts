...
export class AudioGame {
  ...
  createQuestion() {
    ...
    shuffle(answers);
    const answersButtons: HTMLButtonElement[] = Array.from(document.querySelectorAll('.audio-game-buttons > button'));
  ...
  answerQuestion(id: number) {
    ...
    else this.playAudioButton!.classList.add("wrong");
    ...
    (document.querySelector('.audio-game-image') as HTMLImageElement).src =
      `https://rs-lang-team187.herokuapp.com/${this.words[this.currentIndex].image}`;
    (document.querySelector('.audio-word') as HTMLHeadingElement).textContent =
  ...
  endGame(): void {
    document.querySelector('.main')!.innerHTML =
      `<h2>Игра окончена! Вы угадали ${this.points}/20 слов!</h2>
    <button class="audio-new-game">Начать новую игру</button>`
    document.querySelector('.audio-new-game')!.addEventListener('click', () => { this.startNewGame() });
  }
  ...
  `;
    ...
    this.getWords(+this.difficulty, randomPage).then(() => { this.createQuestion() });
    this.playAudioButton.addEventListener('click', () => this.playAudio());
    const answersButtons: HTMLButtonElement[] = Array.from(document.querySelectorAll('.audio-game-buttons > button'));
    answersButtons.forEach(button => button.addEventListener('click', (e) => {
